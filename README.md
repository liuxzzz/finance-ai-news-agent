...
- **AI-first**：AI 负责研究规划、证据补充、事件判断、编辑、校验和记忆巩固。
...
LangGraph 只存在于 `packages/core` 内部。Plugin SDK 不暴露 LangGraph、AI SDK、MCP
SDK 或数据库驱动的类型。
...
```text
Planner → Researcher → Curator → Editor → Critic
                                      ↑       |
                                      └─ 修订 ┘
                                              ↓
                                       Memory Curator
...
完整方案见[技术方案](docs/technical-design.md)。
